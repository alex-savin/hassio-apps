...
- **socat** (uses internal port 8486, mapped to host 8486)
  ```yaml
  enabled: false
...
> [!NOTE]
> The default socat slave listens on internal port `8486`, mapped to host port `8486`. Adjust the `master` and `slave` options per your needs. The Z2M serial port setting must be changed separately.
